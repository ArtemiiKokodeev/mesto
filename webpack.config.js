const path = require('path'); // подключаем path к конфигу вебпак, превращает для Webpack относительные пути в абсолютные
const HtmlWebpackPlugin = require('html-webpack-plugin'); // плагин для HTML
const { CleanWebpackPlugin } = require('clean-webpack-plugin'); // плагин, удаляющий каждый раз при сборке проекта содержимое папки dist
